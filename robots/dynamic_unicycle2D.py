...
class DynamicUnicycle2D:
    ...
    def f(self, X, casadi=False):
        if casadi:
            return ca.vertcat(
                X[3, 0] * ca.cos(X[2, 0]), X[3, 0] * ca.sin(X[2, 0]), 0, 0
            )
        else:
            return np.array(
                [X[3, 0] * np.cos(X[2, 0]), X[3, 0] * np.sin(X[2, 0]), 0, 0]
            ).reshape(-1, 1)
    ...
    def agent_barrier(self, X, obs, robot_radius, beta=1.01):
        ...
        df_dx = self.df_dx(X)
        dh_dot_dx = (
            np.append((2 * self.f(X)[0:2]).T, np.array([[0, 0]]), axis=1)
            + 2 * (X[0:2] - obsX[0:2]).T @ df_dx[0:2, :]
        )
